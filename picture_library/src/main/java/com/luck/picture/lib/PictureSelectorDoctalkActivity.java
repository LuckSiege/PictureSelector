 ...
 */
public class PictureSelectorDoctalkActivity extends PictureBaseActivity implements View.OnClickListener, OnAlbumItemClickListener, OnPhotoSelectChangedListener<LocalMedia>, OnItemClickListener, OnRecyclerViewPreloadMoreListener {

	private static final String TAG = PictureSelectorDoctalkActivity.class.getSimpleName();
	...
	public int getResourceId () {
		return R.layout.picture_doctalk;
	}
	...
	protected void initWidgets () {
		...
		mRecyclerView.addItemDecoration(new GridSpacingItemDecoration(config.imageSpanCount <= 0 ? PictureConfig.DEFAULT_SPAN_COUNT : config.imageSpanCount, ScreenUtils.dip2px(this, 2), false));
		...
		if (!config.isPageStrategy) {
			...
			mRecyclerView.setOnRecyclerViewPreloadListener(PictureSelectorDoctalkActivity.this);
		}
	...
	private void loadAllMediaData () {
		if (PermissionChecker.checkSelfPermission(this, Manifest.permission.READ_EXTERNAL_STORAGE) &&
			...
			PermissionChecker.requestPermissions(this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE}, PictureConfig.APPLY_STORAGE_PERMISSIONS_CODE);
		}
	}
	...
	protected void initCompleteText (int startCount) {
		if (config.selectionMode == PictureConfig.SINGLE) {
			if (startCount <= 0) {
				mTvPictureRight.setText(getString(R.string.picture_cancel));
			}
			else {
				mTvPictureRight.setText(getString(R.string.picture_done));
			}
		}
		else {
			if (startCount <= 0) {
				...
				mTvPictureRight.setText(getString(R.string.picture_done_count, startCount));
			}
		}
	}
	...
	private void manualSaveFolderForPageModel (LocalMedia media) {
		...
		if (allFolder != null) {
			...
			if (count == 0) {
				//noinspection deprecation
				allFolder.setName(config.chooseMode == PictureMimeType.ofAudio() ? getString(R.string.picture_all_audio) : getString(R.string.picture_camera_roll));
