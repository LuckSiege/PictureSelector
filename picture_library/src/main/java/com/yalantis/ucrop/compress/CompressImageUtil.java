...
public class CompressImageUtil {
    ...
    public void compress(String imagePath, CompressListener listener) {
        if (config.isEnablePixelCompress()) {
            try {
                ...
                listener.onCompressError(imagePath, String.format("图片压缩失败,%s", e.toString()));
                e.printStackTrace();
            }
        } else {
        ...
        mhHandler.post(new Runnable() {
            ...
            public void run() {
                if (isSuccess) {
                    ...
                    listener.onCompressError(imagePath, message);
                }
            }
        });
    }
    ...
    public interface CompressListener {
         ...
         * @param msg     失败的原因
         */
        void onCompressError(String imgPath, String msg);
    }
}
